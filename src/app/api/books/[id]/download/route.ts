...
export async function GET(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const fileNameBase = String(book.title || 'book').replace(/[^a-z0-9]+/gi, '_').toLowerCase();

    return new NextResponse(JSON.stringify(payload, null, 2), {
      status: 200,
      headers: {
        'Content-Type': 'application/json',
        'Content-Disposition': `attachment; filename="${fileNameBase}_${book.edition_year || ''}.json"`,
      },
    });
  } catch (e: any) {
