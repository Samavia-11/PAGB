...
export default function EditArticle() {
  ...
  useEffect(() => {
    ...
    setUser(parsedUser);
    loadDraft(parsedUser.id);
  }, [router, draftId]);

  const loadDraft = (userId: number) => {
    try {
      const savedDrafts = JSON.parse(localStorage.getItem(`drafts_${userId}`) || '[]');
      const draft = savedDrafts.find((d: any) => d.id === parseInt(draftId as string));
      
      if (draft) {
        setFormData({
          title: draft.title || '',
          authors: draft.authors || [{ name: '', email: '', role: 'Main Author', contact: '' }],
          institutionalAffiliation: draft.institutionalAffiliation || '',
  ...
  const updateDraft = async () => {
    ...
    try {
      const savedDrafts = JSON.parse(localStorage.getItem(`drafts_${user.id}`) || '[]');
      const draftIndex = savedDrafts.findIndex((d: any) => d.id === parseInt(draftId as string));
      
      if (draftIndex !== -1) {
        savedDrafts[draftIndex] = {
          ...savedDrafts[draftIndex],
        ...
        localStorage.setItem(`drafts_${user.id}`, JSON.stringify(savedDrafts));
            ...
            <div className="mb-8">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-gray-600">Continue editing your draft article</p>
                </div>
                <Link
                  href="/author/drafts"
            ...
            <div className="bg-white rounded-xl shadow-lg border border-gray-100">
              <div className="p-6 space-y-6">
                ...
                <div className="flex flex-wrap gap-4 pt-6 border-t border-gray-200">
                  ...
                  <Link
                    href="/author/drafts"
                    className="text-red-600 hover:text-red-800 px-6 py-3 rounded-lg hover:bg-red-50 transition-colors font-medium"
                  >
                    Cancel
                  </Link>
                </div>
